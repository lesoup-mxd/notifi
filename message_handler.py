...
def process_with_llm(chat_id, user_query=None):
    """Process chat messages with the local LLM and stream the response."""
    ...
    prompt = f"Here are some recent messages:\n\n{chat_context}\n\n"
    
    if user_query:
        prompt += f"My question about these messages: {user_query}"
    else:
        prompt += "Please summarize these messages or identify any important topics."
    
    # This version streams the response directly to the terminal
    print("\n--- Analysis starting ---\n")
    ...
    curl_cmd = [
        ...
        "-d", json.dumps({
            ...
            "stream": True,
            "messages": [
                {"role": "system", "content": "You are an intelligent message analysis assistant that helps users understand their chat history. Your tasks include:\n\n1. Providing concise but thorough summaries of conversations\n2. Identifying important topics, action items, or decisions made\n3. Answering specific questions about the content of messages\n4. Making connections between messages that might not be immediately obvious\n\nWhen analyzing messages:\n- Focus on factual content and key information\n- Note who said what when it's relevant\n- Identify any tasks, deadlines, or commitments mentioned\n- Highlight questions that were asked but not answered\n\nBe concise but thorough in your responses. Present information in an organized manner with clear sections when appropriate.\n\n974218208 is the chat ID of the user you are assisting."},
